...
NSC_category3 = (
    0.972399595786853  # minimum: 0.972399595786853, maximum: 1.14847961165673
)
ESC_category3 = (
    0.934903719163151  # minimum: 0.934903719163151, maximum: 1.15039662982546
)

# category2 -> category_2
NSC_category2 = (
    0.791864633976558  # minimum: 0.791864633976558, maximum: 0.972393932644748
)
ESC_category2 = (
    0.740407828029512  # minimum: 0.740407828029512, maximum: 0.9348999542536
)
...
contri_ESC = np.load(
    "/data/scratch/rdeng/enhancer_project/ipython_notebooks/2D/contri_score/Whole/shap_explanations_ESC.npy"
)
...
inp_ESC = np.load(
    "/data/scratch/rdeng/enhancer_project/ipython_notebooks/2D/contri_score/Whole/inp_ESC.npy"
)
...
NSC_category5_idx = np.where(
    np.logical_and(
        Activity.NSC_log2_enrichment >= NSC_category5,
        Activity.NSC_log2_enrichment <= NSC_max,
    )
)[0]
ESC_category5_idx = np.where(
    np.logical_and(
        Activity.ESC_log2_enrichment >= ESC_category5,
        Activity.ESC_log2_enrichment <= ESC_max,
    )
)[0]
...
def motif_H5ToTable(h5, fasta_group):
    ...
    for pattern_type in ["pos_patterns", "neg_patterns"]:
        ...
        for pattern_name in pattern_names:
            ...
            for idx, s, e in zip(example_idx, start, end):
                data.append(
                    (pattern_type, pattern_name, idx, s, e)
                )  # append start and end to the data list
    df = pd.DataFrame(
        data,
        columns=[
            "pattern_type",
    ...
    merge_df = pd.merge(
        fasta_group, df, right_on="example_idx", left_index=True, how="left"
    )
...
def motif_location(table):
    ...
    new_table = (
        table.copy()
    )  # Make a copy of the input table to avoid modifying it directly
    new_coordinates = []
    for coordinate, motif_start in zip(new_table["location"], new_table["motif_start"]):
        ...
        new_start = (
            midpoint - 500 + motif_start
        )  # Move the start position left by 500 base pairs, add the value of "extension_start", but not past the beginning of the chromosome
        new_end = (
            new_start + 30
        )  # Move the end position right by 500 base pairs and add the value of "extension_start"
...
dic = {"NSC_category5": NSC_category5_idx, "ESC_category5": ESC_category5_idx}

writer = pd.ExcelWriter(
    "/data/scratch/rdeng/enhancer_project/ipython_notebooks/2D/contri_score/5_categories/Category_5/h5Totable/enhancer_motif.xlsx"
)  # Arbitrary output name
for group_name, idx_name in dic.items():
    fasta_group = input_fasta[input_fasta.index.isin(idx_name)].reset_index(drop=True)
    filename = str(
        "/data/scratch/rdeng/enhancer_project/ipython_notebooks/2D/contri_score/5_categories/Category_5/"
        + group_name
        + "/out_"
        + group_name
        + ".h5"
    ...
    motif_table.to_excel(
        writer, sheet_name=os.path.splitext(group_name)[0], index=False
    )
writer.save()
...
filename = str(
    "/data/scratch/rdeng/enhancer_project/ipython_notebooks/2D/contri_score/5_categories/Category_5/5000/NSC_high/out_NSC_high.h5"
)
h5 = h5py.File(filename, "r")
...
tomtom_result = pd.read_table(
    "/data/scratch/rdeng/enhancer_project/ipython_notebooks/2D/contri_score/NSC_ESC_common/5000/common_Ehigh/tomtom_results.txt",
    header=None,
)
...
contri_scores = np.load(
    "/data/scratch/rdeng/enhancer_project/ipython_notebooks/2D/contri_score/NSC_ESC_common/shap_explanations_ESC_high.npy"
)
inps = np.load(
    "/data/scratch/rdeng/enhancer_project/ipython_notebooks/2D/contri_score/NSC_ESC_common/inp_ESC_high.npy"
)
...
NSC_high_idx = np.where(
    np.logical_and(
        Activity.NSC_log2_enrichment >= NSC_top10,
        Activity.ESC_log2_enrichment < ESC_top10,
    )
)[0]
ESC_high_idx = np.where(
    np.logical_and(
        Activity.NSC_log2_enrichment < NSC_top10,
        Activity.ESC_log2_enrichment >= ESC_top10,
    )
)[0]
common_high_idx = np.where(
    np.logical_and(
        Activity.NSC_log2_enrichment >= NSC_top10,
        Activity.ESC_log2_enrichment >= ESC_top10,
    )
...
common_high = input_fasta[input_fasta.index.isin(common_high_idx)].reset_index(
    drop=True
)
...
# %%
select_enhancer = pd.read_csv(
    "/data/scratch/rdeng/enhancer_project/ipython_notebooks/2D/contri_score/NSC_ESC_common/NSC_high_selectedCoordinates.csv"
)
...
select_idx = input_fasta[
    input_fasta["location"].isin(select_enhancer["location"])
].index.tolist()
